...
uniqueness_map = {}  # Mapping of fitness values -> Set[Individual]
population = []
...
def genetic_algorithm():
    global population, pq, highest_fitness, uniqueness_map
    # evaluation
    for individual in population:
        ...
        if fitness > highest_fitness:
            ...
            pq.add(individual)
            uniqueness_map.setdefault(fitness, set()).add(individual)
        # determine uniqueness
        elif individual not in uniqueness_map.get(fitness, set()):
            pq.add(individual)
            uniqueness_map.setdefault(fitness, set()).add(individual)
    _index = randint(0, len(pq) - 1)
    index = randint(0, _index)
    parent = pq[index]
    # mutation
    population = []
